...
function getGifs(){
    ...
    const search_endpoint = `http://api.giphy.com/v1/gifs/search?q=${gif}&api_key=${giphy_api_key}`;

    /* Fetching Data from the API */
    fetch(search_endpoint)
        .then(api_res => api_res.json())
        .then(api_output => { const gif_array = api_output.data.map(gif => gif.images.fixed_width.url);
          
    /* Outputing a <img> element for every GIFs */
            const gifs  = gif_array    
                                .map(gif => `<div class="flex-fill">
                                                    <img src=${gif}>
                                                    </div>` ).join();            
...
const trending_endpoint = `http://api.giphy.com/v1/gifs/trending?api_key=${giphy_api_key}&limit=10`;

fetch(trending_endpoint)
    .then(api_res => api_res.json())
    .then(api_output => { const trending_gifs_array = api_output.data.map(gif => gif.images.fixed_width.url);
